...
public:
   ...
   * Set the content of the message
   *
   * @param data the content of the message. The given data is moved into
   * message.
   */
  SWIFT_RETURNS_INDEPENDENT_VALUE
   ...
   * copies of this buffer will be made. The caller is responsible to ensure the
   * memory buffer is valid until the message has been persisted (or an error is
   * returned).
   */
   ...
   */
  SWIFT_RETURNS_INDEPENDENT_VALUE
  MessageBuilder &setProperty(const std::string &name,
   ...
   * follow these rules: <ol> <li><code>sequenceId >= 0</code> <li>Sequence id
   * for a message needs to be greater than sequence id for earlier messages:
   * <code>sequenceId(N+1) > sequenceId(N)</code>
   * <li>It's not necessary for sequence ids to be consecutive. There can be
   * holes between messages. Eg. the <code>sequenceId</code> could represent an
   * offset or a cumulative size.
   * </ol>
   *
   * @param sequenceId
   ...
   */
  MessageBuilder &
  setReplicationClusters(const std::vector<std::string> &clusters);
