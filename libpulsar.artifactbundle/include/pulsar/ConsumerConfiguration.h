...
// This file was modified by flexlixrup to include the definitions of bridging.h
// and SWIFT_RETURNS_INDEPENDENT_VALUE, as well as reformatted for consistency.
...
public:
   ...
   * consumers will get an error message. In Shared subscription, multiple
   * consumers will be able to use the same subscription name and the messages
   * will be dispatched in a round robin fashion. In Failover subscription, a
   * primary-failover subscription model allows for multiple consumers to attach
   * to a single subscription, though only one of them will be “master” at a
   * given time. Only the primary consumer will receive messages. When the
   * primary consumer gets disconnected, one among the failover consumers will
   ...
   *
   * By default, KeyShared subscription use auto split hash range to maintain
   * consumers. If you want to set a different KeyShared policy, you can set by
   * following example:
   *
  ...
  ConsumerConfiguration &setTypedMessageListener(
    ...
    return setMessageListener(
        [listener, decoder](Consumer &consumer, const Message &msg) {
   ...
   */
  ConsumerConfiguration &
  setConsumerEventListener(ConsumerEventListenerPtr eventListener);
   ...
   * the consumer before the application calls receive(). Using a higher value
   * may potentially increase the consumer throughput at the expense of bigger
   * memory utilization.
   *
   ...
   * message distribution on shared subscription by pushing messages only to the
   * consumers that are ready to process them. Neither receive with timeout nor
   * partitioned topics can be used if the consumer queue size is 0. The
   * receive() function call should not be interrupted when the consumer queue
   * size is 0.
   *
   * The default value is 1000 messages and it is appropriate for the most use
   * cases.
   *
   ...
   *
   * This setting is used to reduce the receiver queue size for individual
   * partitions
   ...
   */
  void setMaxTotalReceiverQueueSizeAcrossPartitions(
   ...
   * needs to be greater than 10 seconds. An Exception is thrown if the given
   * value is less than 10000 (10 seconds). If a successful acknowledgement is
   * not sent within the timeout all the unacknowledged messages are
   * redelivered.
   *
   * Default: 0, which means the the tracker for unacknowledged messages is
   * disabled.
   *
  ...
  /**
   * Set the tick duration time that defines the granularity of the ack-timeout
   * redelivery (in milliseconds).
   *
   * The default value is 1000, which means 1 second.
   *
   * Using a higher tick time reduces
   * the memory overhead to track messages when the ack-timeout is set to a
   * bigger value.
   *
   ...
   * failed message will be redelivered after a fixed timeout. The default is 1
   * min.
   *
   ...
   * request is not sent to broker until the time window reaches its end, or the
   * number of grouped messages reaches limit. Default is 100 milliseconds. If
   * it's set to a non-positive value, ACK requests will be directly sent to
   * broker without grouping.
   *
   ...
   * set to a non-positive value, number of grouped messages is not limited.
   * Default is 1000.
   *
  ...
  /**
   * Set the time duration for which the broker side consumer stats will be
  ...
  /**
   * If enabled, the consumer reads messages from the compacted topics rather
   * than reading the full message backlog of the topic. This means that if the
   * topic has been compacted, the consumer only sees the latest value for each
   * key in the topic, up until the point in the topic message backlog that has
   * been compacted. Beyond that point, message is sent as normal.
   *
   * `readCompacted` can only be enabled subscriptions to persistent topics,
   * which have a single active consumer (for example, failure or exclusive
   * subscriptions). Attempting to enable it on subscriptions to a
   * non-persistent topics or on a shared subscription leads to the subscription
   * call failure.
   *
   * @param readCompacted
  ...
  /**
   * Set the time duration in minutes, for which the PatternMultiTopicsConsumer
   * will do a pattern auto discovery. The default value is 60 seconds. less
   * than 0 will disable auto discovery.
   *
   ...
   * Non-Persistent, or AllTopics. Only used with pattern subscriptions.
   *
   * @param regexSubscriptionMode The default value is `PersistentOnly`.
   */
  ConsumerConfiguration &
  setRegexSubscriptionMode(RegexSubscriptionMode regexSubscriptionMode);
   ...
   */
  void
  setSubscriptionInitialPosition(InitialPosition subscriptionInitialPosition);
   ...
   *
   * By default, some messages are redelivered many times, even to the extent
   * that they can never be stopped. By using the dead letter mechanism,
   * messages have the max redelivery count, when they exceeding the maximum
   * number of redeliveries. Messages are sent to dead letter topics and
   ...
   *
   * @param replicateSubscriptionState whether the subscription status should be
   * replicated
   */
   ...
   */
  ConsumerConfiguration &setProperty(const std::string &name,
   ...
   * Notice: SubscriptionProperties are immutable, and consumers under the same
   * subscription will fail to create a subscription if they use different
   * properties.
   *
   * @param subscriptionProperties all the subscription properties in the
   * provided map
   */
  ...
  /**
   * Set the Priority Level for consumer (0 is the default value and means the
   * highest priority).
   *
   ...
   * chunks of the original message. While consuming chunk-messages, chunks from
   * same message might not be contiguous in the stream and they might be mixed
   * with other messages' chunks. so, consumer has to maintain multiple buffers
   * to manage chunks coming from different messages. This mainly happens when
   ...
   * M2-C2 belong to M2 message.
   *
   * Buffering large number of outstanding uncompleted chunked messages can
   * create memory pressure and it can be guarded by providing this
   * maxPendingChunkedMessage threshold. Once, consumer reaches this threshold,
   * it drops the outstanding unchunked-messages by silently acking or asking
   * broker to redeliver later by marking it unacked. See
   ...
   */
  ConsumerConfiguration &
  setMaxPendingChunkedMessage(size_t maxPendingChunkedMessage);
  ...
  /**
   * Buffering large number of outstanding uncompleted chunked messages can
   * create memory pressure and it can be guarded by providing the
   * maxPendingChunkedMessage threshold. See setMaxPendingChunkedMessage. Once,
   * consumer reaches this threshold, it drops the outstanding
   * unchunked-messages by silently acking if
   * autoAckOldestChunkedMessageOnQueueFull is true else it marks them for
   * redelivery.
   *
   * Default: false
   *
   * @param autoAckOldestChunkedMessageOnQueueFull whether to ack the discarded
   * chunked message
   */
  ...
  /**
   * If producer fails to publish all the chunks of a message then consumer can
   * expire incomplete chunks if consumer won't be able to receive all chunks in
   * expire times. Use value 0 to disable this feature.
   *
   ...
   */
  ConsumerConfiguration &
  setStartMessageIdInclusive(bool startMessageIdInclusive);
   ...
   *
   * It should be noted that this option can only work when the broker side also
   * enables the batch index acknowledgment. See the
   * `acknowledgmentAtBatchIndexLevelEnabled` config in `broker.conf`.
   *
   * Default: false
   *
   ...
   */
  ConsumerConfiguration &
  intercept(const std::vector<ConsumerInterceptorPtr> &interceptors);
   ...
   *
   * By default, when Consumer::acknowledge is called, it won't wait until the
   * corresponding response from broker. After it's enabled, the `acknowledge`
   ...
   * consumer is created. Instead, the consumer waits to fetch messages until
