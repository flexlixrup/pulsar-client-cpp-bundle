...
public:
   ...
   * client instance. Setting this to 0 will disable the limit. By default this
   * is disabled.
   *
  ...
  /**
   * Sets the max number of connection that the client library will open to a
   * single broker. By default, the connection pool will use a single connection
   * for all the producers and consumers. Increasing this parameter may improve
   * throughput when using many producers over a high latency connection.
   *
   * @param connectionsPerBroker max number of connections per broker (needs to
   * be greater than 0)
   */
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ...
  /**
   * @return the max number of connection that the client library will open to a
   * single broker
   */
   ...
   * messages through message listener. Default is 1 thread per Pulsar client.
   *
   ...
   * always be assigned to the same thread.
   *
   * @param threads number of threads
   */
  SWIFT_RETURNS_INDEPENDENT_VALUE
   ...
   * prevent overload on broker. <i>(default: 50000)</i> It should be configured
   * with higher value only in case of it requires to produce/subscribe on
  ...
  /**
   * Max number of lookup redirection allowed on each look request to prevent
   * overload on broker. <i>(default: 20)</i>
   *
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setInitialBackoffIntervalMs(int initialBackoffIntervalMs);
   ...
   * object. The logger factory can only be set once per process. Any subsequent
   * calls to set the logger factory will have no effect, though the logger
   * factory object will be cleaned up.
   */
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setTlsPrivateKeyFilePath(const std::string &tlsKeyFilePath);
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setTlsCertificateFilePath(const std::string &tlsCertificateFilePath);
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setTlsTrustCertsFilePath(const std::string &tlsTrustCertsFilePath);
  ...
  /**
   * Configure whether the Pulsar client accepts untrusted TLS certificates from
   * brokers.
   *
   * The default value is false.
   *
  ...
  /**
   * @return whether the Pulsar client accepts untrusted TLS certificates from
   * brokers
   */
  bool isTlsAllowInsecureConnection() const;

  /**
   * Configure whether it allows validating hostname verification when a client
   * connects to a broker over TLS.
   *
   * It validates the incoming x509 certificate and matches the provided
   * hostname (CN/SAN) with the expected broker's hostname. It follows the
   * server identity hostname verification in RFC 2818.
   *
   * The default value is false.
   *
   ...
   * If a partitioned topic is produced or subscribed and `intervalInSeconds` is
   * not 0, every `intervalInSeconds` seconds the partition number will be
   * retrieved by sending lookup requests. If partition number has been
   * increased, more producer/consumer of increased partitions will be created.
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setPartititionsUpdateInterval(unsigned int intervalInSeconds);
   ...
   * established. If the duration passes without a response from the broker, the
   ...
   * proxy. Client must configure both proxyServiceUrl and appropriate
   * proxyProtocol.
   *
   ...
   * Pulsar supports SNI proxy routing.
   *
   * SNI routing:
  ...
  /**
   * Set keep alive interval for each client-broker-connection. <i>(default: 30
   * seconds)</i>.
   *
  ...
  SWIFT_RETURNS_INDEPENDENT_VALUE
  ClientConfiguration &
  setKeepAliveIntervalInSeconds(unsigned int keepAliveIntervalInSeconds);
...
private:
  ...
  // By default, when the client connects to the broker, a version string like
  // "Pulsar-CPP-v<x.y.z>" will be carried and saved by the broker. The client
  // version string could be queried from the topic stats.
  //
  // This method provides a way to add more description to a specific `Client`
  // instance. If it's configured, the description will be appended to the
  // original client version string, with '-' as the separator.
  //
  // For example, if the client version is 3.2.0, and the description is
  // "forked", the final client version string will be
  // "Pulsar-CPP-v3.2.0-forked".
  //
  // NOTE: This method should only be called by the PulsarWrapper and the length
  ...
  // ```
  //
  // Then, call the method before passing the `conf` to the constructor of
  // `Client`:
  //
  // ```c++
